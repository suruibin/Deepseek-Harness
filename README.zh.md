...
- **背景壁纸**：同一下方新增「背景壁纸」——点「选择壁纸」弹系统文件对话框选本地图片（png/jpg/webp/gif/svg/bmp，≤12 MiB），图片存入 userData 并以 data URL 注入页面底层（玻璃 tint 之下）：固定铺满、轻度模糊 + 压暗保证文字可读，透明度滑块仍控制玻璃浓淡（alpha 越低壁纸越清晰）；「移除」恢复默认玻璃效果。选择经主进程持久化到 `glass-settings.json`。
- **桌面功能开关**：主题设置面板下方新增「桌面功能」——「显示浏览文件夹」与「显示终端」两个使能开关控制文件面板/终端 dock 及其浮动按钮的显隐（关闭即收起并隐藏，重新打开恢复），「标题颜色切换时间」设置左上角 DeepSeek 标志与图标换色的间隔（1–600 秒，默认 10 秒）。选择持久化到 localStorage 并即时生效（`featureControlScript` 与 terminal/ambient 脚本通过 window 事件联动）。
