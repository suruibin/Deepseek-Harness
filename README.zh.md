...
- **界面毛玻璃(统一入口)**：主题设置面板**最顶部**「界面毛玻璃」——「主界面毛玻璃」滑块(20–60%,默认 35%)一键统一输入框、任务条、消息气泡与输入时弹出的模型/模式/命令菜单的玻璃浓淡(这些弹出菜单原为不透明深灰 `rgb(39,46,62)`,现与输入框同款毛玻璃)；「设置界面毛玻璃」滑块独立控制设置面板自身的玻璃浓淡(原为不透明 `rgb(32,38,52)`)。两个滑块经 CSS 变量即时作用于各自界面族,无需逐个界面单独设置,持久化到 localStorage。缩略图解码在渲染端串行排队 + 主进程按 mtime 缓存,避免打开面板时解码多张壁纸大图阻塞主进程(切换卡顿,原峰值 756ms → 现 ~9ms)。
