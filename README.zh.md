...
- **主题设置**：设置侧边栏新增独立「主题设置」菜单项（通用设置之后），将背景透明度滑块、光标特效与背景壁纸三个控件从 通用设置 → 外观 迁入其中；主题设置面板打开时原生通用设置内容自动隐藏，切换其它菜单项自动恢复（`src/glass.ts` 的 `themeSettingsScript`，注入按钮位于导航列表之外以避免干扰 React 的列表渲染）。
- **背景壁纸**：主题设置面板首块「背景壁纸」——「选择文件夹…」打开系统目录对话框,以 **3 列 × 2 行**缩略图网格列出文件夹内图片(png/jpg/jpeg/webp/gif/bmp),**双击缩略图**即切换壁纸,更多图片通过网格内竖直滑块浏览(缩略图按需懒加载);「选择壁纸…」保留单张选择,「移除」恢复默认玻璃效果。图片存入 userData 并以 data URL 注入页面底层(玻璃 tint 之下)：固定铺满、轻度模糊 + 压暗保证文字可读,透明度滑块仍控制玻璃浓淡(alpha 越低壁纸越清晰)。选择经主进程持久化到 `glass-settings.json`,上次浏览的文件夹会记住并在下次打开时自动恢复。
- **桌面功能开关**：主题设置面板下方「桌面功能」——「显示浏览文件夹」与「显示终端」两个使能开关控制文件面板/终端 dock 及其浮动按钮的显隐(关闭即收起并隐藏,重新打开恢复)；其上方独立一行的「标题颜色切换时间」(位于背景透明度之前)设置左上角 DeepSeek 标志与图标换色的间隔(1–600 秒,默认 10 秒)。选择持久化到 localStorage 并即时生效(`featureControlScript` 与 terminal/ambient 脚本通过 window 事件联动)。
- **环境装饰(主题设置)**:主题设置面板「桌面功能」下方新增「环境装饰」分组——**粒子鲸鱼**(deepseek.com/harness 官网同款粒子鱼,居中于对话区,深色白粒子/浅色灰粒子)、**环境生物**(品牌鱼剪影游动、气泡上浮、浮游生物闪烁)与**交互网格**(官网 dot-grid 点阵,指针 140px 内弹簧斥力);以及「悬停效果」分组——**鼠标辉光**(蓝紫径向光斑跟随光标,透过半透明玻璃面板弥散)与**悬停下压**(面板随光标几何倾斜约 1° 带回弹)。所有效果渲染在壁纸之上、应用内容之下(`#dsh-dt-ambient`,pointer-events 穿透),开关即时生效并持久化到 localStorage,默认全开。设置面板打开时侧边栏自动暂停下压,避免 transform 把面板的 fixed overlay 重新锚定进列里(`src/ambient.ts`,移植自 DSH-Transparent-UI-Plugin)。
- **界面毛玻璃(统一入口)**：主题设置面板**最顶部**「界面毛玻璃」——「主界面毛玻璃」滑块(20–60%,默认 35%)一键统一**整个主界面**(排除设置界面)：侧边栏、中间输入显示栏、文件浏览器、终端 dock、输入框、消息气泡与输入时弹出的模型/模式/命令菜单,全部同一套玻璃浓淡(这些表面原为半透明无模糊或完全不透明深灰);「设置界面毛玻璃」滑块独立控制设置面板自身的玻璃浓淡。两个滑块经 CSS 变量即时作用于各自界面族,无需逐个界面单独设置,持久化到 localStorage。缩略图解码在渲染端串行排队 + 主进程按 mtime 缓存,避免打开面板时解码多张壁纸大图阻塞主进程(切换卡顿,原峰值 756ms → 现 ~9ms)。
...
- **插件机制**：`<appPath>/plugins/`（随包内置）与 `<userData>/plugins/`（用户目录，同名覆盖内置）下的 `*.css` / `*.js` 文件在页面加载后自动注入，定制外观**无需重新编译**（详见 `plugins/README.md`）。内置插件 `settings-icon.css` 修复侧边栏收起态设置图标不可见（SVG path 无 fill 属性时回退到初始黑色，改用 `currentColor` 跟随按钮文字色）。

## 测试

```sh
npm test        # 56 个无密钥用例：命令解析、就绪行解析、HTTP 轮询、PTY 注册表、git status、插件加载合并
npm run typecheck
```
