...
if echo "$SRC" | grep -q 'DankMaterialShell/plugins/'; then
  ...
  # (or a missing marker) copies everything. node_modules/.git stay excluded.
  if [ "$MODE" = "all" ] || [ ! -f "$MARK" ]; then
    rsync -a "${EXCLUDES[@]}" "$SRC"/ "$DEST"/ 2>/dev/null
  else
    mapfile -t FILES < <(find "$SRC" -newer "$MARK" -type f ! -path '*/node_modules/*' ! -path '*/.git/*' 2>/dev/null | sed "s|^$SRC/||")
    if [ "${#FILES[@]}" -gt 0 ]; then
...
RECORD="$DEST/修改记录.txt"
{
  ...
  printf '%s\n' '其他修改:'
  if [ "$MODE" = "all" ] || [ ! -f "$MARK" ]; then
    find "$SRC" -type f ! -path '*/node_modules/*' ! -path '*/.git/*' 2>/dev/null | sed "s|^$SRC/||" | head -200
  else
    find "$SRC" -newer "$MARK" -type f ! -path '*/node_modules/*' ! -path '*/.git/*' 2>/dev/null | sed "s|^$SRC/||" | head -200
  fi
} > "$RECORD" 2>/dev/null
