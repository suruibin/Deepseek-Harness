...
- **Unified interface glass**: at the TOP of the Theme Settings panel, the "界面毛玻璃 / Interface glass" block has two independent sliders (20–60%, default 35%) — "主界面毛玻璃 / Main surface" re-themes the composer card, task strip, user bubbles and the popup menus that appear while typing (model/access-mode/command pickers, formerly opaque `rgb(39,46,62)`) at once; "设置界面毛玻璃 / Settings surface" re-themes the hosted settings panel itself (formerly opaque `rgb(32,38,52)`). Both apply immediately via CSS variables and persist to localStorage. Thumbnail decodes are serialized in the renderer and cached by mtime in the main process, so opening the panel no longer stalls the window (switch latency dropped from ~756ms to ~9ms).
