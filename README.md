...
- **Desktop feature toggles**: below the wallpaper control, the Theme Settings panel (主题设置) gained a "Desktop features" block — a show/hide switch for the file browser and a show/hide switch for the terminal dock (turning one off hides both the panel and its floating toggle button); above the opacity slider sits the brand color-switch interval input (1–600 s, default 10 s). Choices persist to localStorage and apply immediately via window events (`featureControlScript` ↔ `terminalScript`/`ambientStyleScript`).
- **Wallpaper folder grid**: the wallpaper control gained a "选择文件夹… / Choose folder…" button that lists the folder's images as a 3-column × 2-row thumbnail grid (lazy-loaded), scrollable vertically for more; double-clicking a thumbnail applies that wallpaper (the last browsed folder is remembered). The single-file "选择壁纸…" picker and "移除" remain.
- **Ambient decoration (Theme Settings)**: below "桌面功能 / Desktop features" in the Theme Settings panel, an "环境装饰 / Ambient decoration" group — **particle whale** (the deepseek.com/harness hero fish rendered as particles, centered on the conversation column, white particles in dark mode / gray in light), **marine life** (brand-fish silhouettes swimming, rising bubbles, twinkling plankton) and **interactive mesh** (the site's dot-grid with pointer repel); plus a "悬停效果 / Hover effects" group — **cursor glow** (a blue radial light following the pointer, diffusing through the translucent glass panes) and **hover press** (the pane under the cursor tilts ~1° with an ease-back). Everything renders above the wallpaper and below the app content (`#dsh-dt-ambient`, pointer-events pass through); the toggles apply immediately, persist to localStorage, and default to on. While the settings panel is open the sidebar pauses its tilt so the transform never re-anchors the panel's fixed overlay into the column (`src/ambient.ts`, ported from DSH-Transparent-UI-Plugin).
- **Unified interface glass**: at the TOP of the Theme Settings panel, the "界面毛玻璃 / Interface glass" block has two independent sliders (20–60%, default 35%) — "主界面毛玻璃 / Main surface" re-themes the WHOLE main UI (everything except the settings panel) at once: the sidebar, the center conversation column, the file browser, the terminal dock, the composer card, user bubbles and the popup menus that appear while typing (formerly translucent-without-blur or opaque). "设置界面毛玻璃 / Settings surface" independently re-themes the hosted settings panel. Both apply immediately via CSS variables and persist to localStorage. Thumbnail decodes are serialized in the renderer and cached by mtime in the main process, so opening the panel no longer stalls the window (switch latency dropped from ~756ms to ~9ms).
